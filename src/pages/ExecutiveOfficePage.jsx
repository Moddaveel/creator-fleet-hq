...
const ALL_DEPARTMENTS = [
  {
    ...
    color:"#a855f7",
    nav:"executive",
    description:"Routes tasks, tracks KPIs, generates briefings, and acts as the primary creator interface.",
    agents:EO_AGENTS
  },
  {
    id:"brand",
  ...
  return (
    <div style={{ padding:24, maxWidth:1100, margin:"0 auto" }}>
      ...
      <div style={{ marginBottom:16 }}>
        <button onClick={() => setAlertsOpen(o => !o)} style={{ width:"100%", background:C.card, border:"1px solid "+C.border, borderRadius:alertsOpen?"12px 12px 0 0":12, padding:"10px 16px", display:"flex", alignItems:"center", justifyContent:"space-between", cursor:"pointer", color:C.text }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:12, marginBottom:20 }}>
        {statCards.map((s,i) => (
          <div key={i} onClick={s.onClick} style={{ background:s.bg, border:"1px solid "+s.border, borderRadius:14, padding:18, cursor:"pointer", transition:"all 0.15s" }}>
              ...
              <div key={dept.id} style={{ borderBottom:"1px solid "+C.border }}>
                ...
                {isOpen && (
                  <div style={{ padding:"0 20px 14px 20px", display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(240px,1fr))", gap:8 }}>
                    {dept.agents.map(a => (
                      <div key={a.id} onClick={() => setChatAgent(a)} style={{ background:a.color+"0d", border:"1px solid "+a.color+"33", borderRadius:10, padding:"12px 14px", cursor:"pointer", display:"flex", gap:12, alignItems:"flex-start" }}>
