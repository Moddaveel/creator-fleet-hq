...
const NAV = [
  ...
  {id:"executive",    label:"The Collective",  icon:"⚡"},
];
...
export default function App() {
  ...
  // Scroll-to-top ref — attached to the main content container
  ...
  const handleApproveAndSchedule = (item, platforms) => {
    setPublishQueue(q => [...q, {
      pub_id: "pub_"+Date.now(),
      source_id: "approval_"+item.id,
      title: item.title,
      type: item.type,
      addedFrom: "Approval Queue",
      addedTs: Date.now(),
      platforms,
    }]);
  };
    ...
    <div style={{ background:C.bg, minHeight:"100vh", color:C.text, fontFamily:"'Inter',system-ui,sans-serif", fontSize:14 }}>
      ...
      <div style={{ background:"#1a1025", borderBottom:"1px solid "+C.border, padding:"0 24px", display:"flex", alignItems:"center", justifyContent:"space-between", height:54, position:"sticky", top:0, zIndex:200 }}>
        ...
        <div style={{ display:"flex", gap:8 }}>
          {urgentN>0 && (
            <div onClick={()=>navigateTo("approvals")} style={{ background:C.red+"22", border:"1px solid "+C.red+"44", borderRadius:8, padding:"5px 12px", color:C.red, fontSize:12, fontWeight:700, cursor:"pointer" }}>
              {"🔴 "+urgentN+" Urgent"}
            </div>
          )}
          {scheduledN>0 && (
            <div onClick={()=>navigateTo("publishing")} style={{ background:C.blue+"22", border:"1px solid "+C.blue+"44", borderRadius:8, padding:"5px 12px", color:C.blue, fontSize:12, fontWeight:700, cursor:"pointer" }}>
              {"📅 "+scheduledN+" Scheduled"}
            </div>
          )}
        </div>
      </div>
      ...
      <div style={{ background:"#1e1333", borderBottom:"1px solid #6b21a8", padding:"0 20px", display:"flex", position:"sticky", top:54, zIndex:199, overflowX:"auto" }}>
        {NAV.map(n => {
          ...
          const badge = n.id==="approvals"&&approvals.length>0 ? approvals.length : n.id==="publishing"&&draftPubN>0 ? draftPubN : null;
          const badgeColor = n.id==="approvals" ? C.red : C.yellow;
          return (
            <button key={n.id} onClick={()=>navigateTo(n.id)}
              style={{ background:active?C.purple+"33":hov?"#a855f722":"transparent", border:"none", borderBottom:"2px solid "+(active?C.purple:hov?C.purple+"88":"transparent"), color:active?C.purple:hov?C.text:C.muted, padding:"11px 14px", fontSize:12, fontWeight:active?700:400, cursor:"pointer", display:"flex", alignItems:"center", gap:5, whiteSpace:"nowrap", transition:"all 0.15s ease" }}
              onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}>
              <span>{n.icon}</span>
              <span>{n.label}</span>
              {badge && <span style={{ background:badgeColor, color:"#fff", borderRadius:10, padding:"1px 6px", fontSize:10, fontWeight:800 }}>{badge}</span>}
            </button>
          );
        })}
      </div>

      {/* Page content — ref here for scroll reset */}
      <div ref={contentRef} style={{ overflowY:"auto", height:"calc(100vh - 112px)" }}>
