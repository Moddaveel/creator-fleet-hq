...
const ALL_DEPARTMENTS = [
  {
    id:"executive", label:"The Collective", icon:"⚡", color:"#a855f7", nav:"executive",
    description:"The executive brain — routes tasks, tracks KPIs, generates briefings, and acts as the primary creator interface.",
    agents: EO_AGENTS,
  },
  {
    id:"brand", label:"Brand Strategy", icon:"🎯", color:"#d946ef", nav:"brand",
    description:"Maintains doctrine, voice, positioning, and brand QA across all content.",
    agents: BS_AGENTS,
  },
  {
    id:"studio", label:"Content Studio", icon:"🎬", color:"#a855f7", nav:"studio",
    description:"Produces and packages clips — from VOD ingestion to platform-ready bundles.",
    agents: CS_AGENTS,
  },
  {
    id:"monetization", label:"Monetization", icon:"💰", color:"#22c55e", nav:"monetization",
    description:"Manages the full deal pipeline from prospecting to sponsorship packages and outreach.",
    agents: MONO_AGENTS,
  },
  {
    id:"publishing", label:"Publishing", icon:"🚀", color:"#3b82f6", nav:"publishing",
    description:"The last mile — scheduling, formatting, copy optimisation, and performance tracking.",
    agents: PUB_AGENTS,
  },
];
...
export default function ExecutiveOfficePage({ approvals, navigateTo }) {
  ...
  const statCards = [
    { label:"Agents",        val:"23",             color:C.purple, bg:"#18061a", border:"rgba(168,85,247,0.45)", icon:"🤖", onClick:() => setRosterOpen(o=>!o) },
                  ...
                  <div style={{ padding:"0 20px 14px 20px", display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(240px,1fr))", gap:8 }}>
                    {dept.agents.map(a => (
                      <div key={a.id} onClick={() => setChatAgent(a)}
                        ...
                        <div style={{ width:38, height:38, background:a.color+"20", border:"1px solid "+a.color+"44", borderRadius:9, display:"flex", alignItems:"center", justifyContent:"center", fontSize:20, flexShrink:0 }}>
                          {a.icon}
                        </div>
